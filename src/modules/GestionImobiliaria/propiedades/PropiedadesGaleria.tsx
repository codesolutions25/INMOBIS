...
import { PropiedadImagen, getPropiedadImagenes } from '@/services/apiPropiedadImagenes';
...
import styles from './styles/PropiedadGaleria.module.css';

interface PropiedadesGaleriaProps {
  idPropiedad: number;
}
...
const getCorrectImageUrl = (url: string): string => {
  ...
  // Si la URL parece ser un nombre de archivo, usarla directamente
  return url;
};

export default function PropiedadesGaleria({ idPropiedad }: PropiedadesGaleriaProps) {
    ...
    useEffect(() => {
      const loadImagenes = async () => {
          ...
          try {
            const response = await getPropiedadImagenes(idPropiedad, 1, 20); // Cargar más imágenes para la galería
      ...
      if (imagenes.length === 0) {
        return (
          <div className={styles.gallery__empty}>
            ...
            <p className={styles.gallery__emptyText}>No hay imágenes disponibles para esta propiedad</p>
          </div>
        );
      }
      ...
      return (
          <div className={styles.gallery}>
            <div className={styles.gallery__div}>
              ...
              >
                {imagenes.map((imagen) => (
                  <SwiperSlide key={imagen.idPropiedadImagenes}>
                    <div className={styles.gallery__mainImageWrapper}>
                      <img 
                        src={getCorrectImageUrl(imagen.urlImagen)} 
                        alt={imagen.descripcion || "Imagen de la propiedad"} 
              ...
              >
                {imagenes.map((imagen) => (
                  <SwiperSlide key={`thumb-${imagen.idPropiedadImagenes}`}>
                ...
                {imagenes.length > 0 && (
                  <div className={styles.gallery__infoDescription}>
                    <p className={styles.gallery__infoText}>
                      {imagenes[activeIndex]?.descripcion || "Imagen de la propiedad"}
                    </p>
                  </div>
                )}
