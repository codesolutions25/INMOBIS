...
import { ProyectoImagen, getProyectoImagenes, deleteProyectoImagenFromProyecto, setImagenPrincipal } from '@/services/apiProyectoImagenes';
...
import styles from './styles/ProyectoImageList.module.css';
...
export default function ProyectoImagenesList({ proyecto, onRefresh }: ProyectoImagenesListProps) {
  ...
  const loadImagenes = async (page = 1) => {
    ...
    try {
      console.log('Cargando imágenes para el proyecto:', proyecto.idProyectoInmobiliario);
      const response = await getProyectoImagenes(proyecto.idProyectoInmobiliario, page, limit);
      ...
      if (response && response.data && Array.isArray(response.data)) {
        ...
        if (response.data.length > 0) {
          ...
          console.log('ID de la primera imagen:', primeraImagen.idProyectosImagenes);
  ...
  useEffect(() => {
    loadImagenes();
  }, [proyecto.idProyectoInmobiliario, page, limit]);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      // 1. Primero eliminamos la imagen del registro de Proyecto-Imágenes
      await deleteProyectoImagenFromProyecto(proyecto.idProyectoInmobiliario, idImagen);
      
      // 2. Luego intentamos eliminar la imagen del microservicio de archivos
      try {
        ...
        console.error('Error al eliminar el archivo físico:', fileErr);
        // No bloqueamos el flujo si falla la eliminación del archivo físico
      }
      
      // 3. Actualizamos la lista de imágenes
      loadImagenes(page);
  ...
  // Esta función se llama cuando se sube una imagen exitosamente
      ...
      {loading ? (
        ...
        <>
          <div className={styles.imageList__grid}>
            {imagenes.map(imagen => (
              <div key={imagen.idProyectosImagenes} className={styles.imageItem}>
                ...
                <div className={styles.imageItem__deleteButtonWrapper}>
                  <Button 
                    variant="destructive" 
                    size="sm"
                    disabled={isDeleting !== null}
                    onClick={() => handleDeleteImageConfirmation(imagen.idProyectosImagenes, imagen.urlImagen)}
                    className={styles.imageItem__deleteButton}
                  >
                    {isDeleting === imagen.idProyectosImagenes ? (
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction 
              className={styles.deleteDialog__confirmButton}
            >
